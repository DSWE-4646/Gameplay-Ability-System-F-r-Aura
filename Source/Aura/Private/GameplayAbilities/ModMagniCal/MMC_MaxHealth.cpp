...
#include "GameplayAbilities/ModMagniCal/MMC_MaxHealth.h"
...
float UMMC_MaxHealth::CalculateBaseMagnitude_Implementation(const FGameplayEffectSpec& Spec) const
{
	...
	Vigor = FMath::Max(Vigor, 0.0f); //确保Vigor不小于0
	...
	//自定义计算逻辑：MaxHealth = 80 + 2.5*Vigor + 10*PlayerLevel
	const float BaseValue = 80.f;
    const float VigorMultiplier = 2.5f;
    const float LevelMultiplier = 10.f;
    return BaseValue + (VigorMultiplier * Vigor) + (LevelMultiplier * PlayerLevel);
}
